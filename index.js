...
async function verifyPayment(txHash) {
  try {
    ...
    const wallet = process.env.SONIC_USDC_WALLET.toLowerCase();
...
async function getLiveRates() {
  try {
    const [ecbRes, cgRes] = await Promise.all([
      ...
      fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,sonic-2,binancecoin,matic-network,chainlink,near,cosmos,ripple,algorand,celo&vs_currencies=usd"),
    ]);
    ...
    const crypto = {
      ...
      SOL: cg["solana"]?.usd, AVAX: cg["sonic-2"]?.usd,
...
<html><head><title>Bercy Sonic Pay</title>
<style>
...
<h1>🔺 Bercy Sonic Pay</h1>
<p>World's first <b>x402 + AC2</b> cross-border payment layer on Sonic Mainnet.</p>
<div>
  <span class="badge">⚡ x402 on Sonic</span>
...
<hr>
<p>🔺 Sonic C-Chain | Chain 146 | x402 + AC2<br>
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ status: "ok", service: "Bercy Sonic Pay", network: "Sonic Mainnet", protocols: ["x402", "AC2"], totalCorridors: 95 });
  }
});
...
app.post("/api/authorize", async (req, res) => {
  ...
  const approval_id = "ac2_sonic_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  res.json({
    approved: true,
    approval_id,
    from, to, amount,
    ...
    message: "AC2: Human approval granted. Proceed with bercy_pay_sonic.",
    timestamp: new Date().toISOString()
  });
});
...
app.post("/api/orchestrate", async (req, res) => {
  ...
  if (!payment) return res.status(402).json({
    ...
    pay_to: process.env.SONIC_USDC_WALLET,
    message: "Send $0.10 USDC on Sonic C-Chain, include tx hash as X-PAYMENT header"
  });
  ...
  const tag = "sonic_" + Date.now().toString(36);
  res.json({
    success: true,
...
app.listen(PORT, () => console.log("Bercy Sonic Pay running on port " + PORT));
